...
function Update_test_admin(props) {
    ...
    useEffect(() => {
        axios.get('/test/' + searchParams.get("id"), {
            ...
            setErrors({ name: "", questions: Array(response.data.questions.length).fill(''), creation: "" }) //сколько вопросов, столько и ошибок для каждого вопроса
        })
            .catch((error) => {
                console.log(error);
            })
...
export default Update_test_admin;